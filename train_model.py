...
class PendulumNetwork(nn.Module):
    ...
    def __init__(
        ...
        """
        super(PendulumNetwork, self).__init__()
...
def pendulum_loss(prediction, actual):
    angle_loss = -F.cosine_similarity(prediction[:, :-1], actual[:, :-1])
    vel_loss = F.mse_loss(prediction[:, -1], actual[:, -1])
...
def train_model(train_data, valid_data, use_cuda=True):
    ...
    model = PendulumNetwork().to(device).to(torch.double)
        ...
        for e in range(NUM_EPOCHS):
            with torch.no_grad():
                ...
                for batch_inputs, batch_outputs in validloader:
                    ...
                    valid_loss = pendulum_loss(pred_outputs, batch_outputs)
                    valid_losses.append(valid_loss.mean().to("cpu"))

            train_losses = []
            for batch_inputs, batch_outputs in trainloader:
                pred_outputs = model.forward(batch_inputs)
                train_loss = pendulum_loss(pred_outputs, batch_outputs)
                optimizer.zero_grad()
                train_loss.backward()
                optimizer.step()
...
def train_predictor(
    ...
    train_trajs = simulator.rad_to_cossin(train_trajs)
    valid_trajs = simulator.rad_to_cossin(valid_trajs)
